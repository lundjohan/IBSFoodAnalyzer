...
public class TagPointPortionsHandler {
     ...
     * tag in chunk.
     *
     * @param hours, refers to the coherent time that must have been fulfilled
     *               before
     * @throws Exception
     */
    static void addPortionScore(Chunk chunk, Map<String, TagPoint> tagPoints,
        ...
        //quant is for "portionTagPoints" hours of scoreTime
        double quantity = 0;
        ...
        while (true) {
            ...
            LocalDateTime endOfScorePeriod = getEndOfScorePeriod(meals, startOfLookBehind,
                    startOfScorePeriod, TPUtil.getEndTime(chunk), portionSize);
            if (endOfScorePeriod == null) {
                break;
            }
            double durationInMinutesOfScorePeriod = ((endOfScorePeriod.atZone(ZONE_ID)
                    .toEpochSecond() -
            ...
            if (durationInMinutesOfScorePeriod > 0) {
                ...
                score += TPUtil.calcAccumulatedScoreFromToTime(chunk, startOfScorePeriod, (long)
                        durationInMinutesOfScorePeriod);
            }
            //for next loop
    ...
    private static LocalDateTime getEndOfScorePeriod(List<Meal> meals,
                                                     LocalDateTime startOfLookBehind,
                                                     LocalDateTime startOfScorePeriod,
                                                     LocalDateTime endOfChunk, long portionSize) {
        if (meals.isEmpty()) {
            return null;
        }
